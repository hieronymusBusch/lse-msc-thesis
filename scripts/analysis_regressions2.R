...
# binary outcomes
cross_section_plot(var = "edu_cur", ylab = "Logit-Coefficient of Treatment Dummy", scale = 4, class = "binary")
ggsave(here::here("plots/figure_cross_edu.pdf"), width = 14.2, height = 13.4, units = "cm")

cross_section_plot(var = "empl", ylab = "Logit-Coefficient of Treatment Dummy", scale = 4, class = "binary")
...
# continous
df_linc <- panel_table(list_df, var = "linc", class = "cont", out = "df")
df_health <- panel_table(list_df, var = "health", class = "cont", out = "df")
df_lifesat <- panel_table(list_df, var = "lifesat", class = "cont", out = "df")

df_cont <- cbind(df_linc, cbind(df_health[, c(2,3)], df_lifesat[, c(2,3)]))
stargazer(df_cont, type = "latex", summary = FALSE)

# binary
df_edu <- panel_table(list_df, var = "edu_cur", class = "binary", out = "df")
df_empl <- panel_table(list_df, var = "empl", class = "binary", out = "df")

df_binary <- cbind(df_edu, df_empl[, c(2,3)])
stargazer(df_binary, type = "latex", summary = FALSE)


stargazer(lm(linc ~ age, dfmain), lm(linc ~ age + sampreg, dfmain), lm(linc ~ age + sampreg + mig, dfmain))
